...
(function ($) {
  ...
  meth.init = function (options) {
    ...
    $form.addClass('sendmail-form').on('submit' + '.sml-' + _.nsid, function () {
      _.send.call(_);
      return false;
    });
  ...
  meth.send = function () {
    ...
    if (!$form.find('input[type="file"]').length) {
      ...
      if (_.addData) fd.data += '&' + $.param(_.addData);
      ...
      if (!window.FormData) {
        $form.off('submit.sml-' + _.nsid).trigger('submit');
      }
    }
  ...
  meth.reinit = function (newSets) {
    var _ = this,
      $form = $(_.form),
      sets = typeof newSets == 'object' && Object.keys(newSets).length != 0 ? newSets : $.extend(true, {}, _.settings);
  ...
  $.fn.sendMail = function () {
    ...
    $.each(this, function (i, it) {
      if (typeof mth == 'object' || typeof mth == 'undefined') it[pn] = crtInst(it, mth);
      else if (mth === 'init' || mth === 'reinit') it[pn] ? getMeth(it, mth, args) : (it[pn] = crtInst(it, args[1]));
